(function () {
  ...
  const instructionsHeaderEl = document.getElementById('instructionsHeaderStt');
  const instructionsWrapperEl = document.getElementById('instructionsSttWrapper');
  const instructionsContentEl = document.getElementById('instructionsStt');
  ...
  function renderInstructions() {
    if (!instructionsContentEl) return;

    const warningHTML = `
      <div class="warning-banner">
        <div class="warning-title">주의사항</div>
        <ul>
          <li>이 페이지는 Web Speech API(음성 인식)와 Web Serial/Web Bluetooth(디바이스 연결)를 함께 사용합니다.</li>
          <li>Chrome/Edge 최신 버전 사용을 권장하며, 브라우저 정책에 따라 일부 기능이 제한될 수 있습니다.</li>
          ...
          <li>iPhone의 경우 Bluefy App에서 STT지원이 불가하여 사용이 제한됩니다.</li>
          <li>PC의 블루투스 하드웨어와 드라이버가 정상 동작해야 합니다.</li>
          <li>음성 인식 시작 전에 브라우저의 마이크 권한을 반드시 허용하세요.</li>
          <li>인식 문구와 전송 데이터는 최대 15자까지만 입력할 수 있습니다.</li>
        </ul>
      </div>
    `;

    const steps = [
      '상단에서 디바이스를 선택하고 연결 방식을 설정합니다.',
      '연결 버튼으로 디바이스를 먼저 연결합니다.',
      '필요하면 아래 "음성 인식 문구 관리"에서 문구와 전송 데이터를 편집합니다.',
      '인식 언어를 선택한 뒤 "음성 인식 시작" 버튼을 누르고 명령을 말합니다.',
      '인식된 문구가 매핑과 일치하면 전송 데이터가 자동으로 디바이스로 전송됩니다.',
      '작업이 끝나면 "음성 인식 종료" 또는 연결 해제로 세션을 종료합니다.',
    ];

    const stepsHTML = '<h3>사용 방법</h3><ol>' + steps.map((step) => `<li>${step}</li>`).join('') + '</ol>';
    instructionsContentEl.innerHTML = warningHTML + stepsHTML;
  }

  function setupInstructionsToggle() {
    if (!instructionsHeaderEl || !instructionsWrapperEl) return;

    const visibleStored = localStorage.getItem('stt.instructions.visible');
    const isOpen = visibleStored === null ? true : visibleStored === 'true';

    instructionsWrapperEl.style.display = isOpen ? 'block' : 'none';
    instructionsHeaderEl.classList.toggle('open', isOpen);

    instructionsHeaderEl.addEventListener('click', () => {
      const nextOpen = instructionsWrapperEl.style.display === 'none';
      instructionsWrapperEl.style.display = nextOpen ? 'block' : 'none';
      instructionsHeaderEl.classList.toggle('open', nextOpen);
      localStorage.setItem('stt.instructions.visible', String(nextOpen));
    });
  }
  ...
  function scheduleSttRestart() {
    if (!sttIsListening || sttIsRecognizing) return;
  ...
  function startSpeechRecognition() {
    ...
    sttRecognition.onerror = (err) => {
      ...
      if (err.error === 'no-speech' || err.error === 'aborted') {
        if (sttIsListening) scheduleSttRestart();
        return;
      }
    ...
    sttRecognition.onend = () => {
      sttIsRecognizing = false;
      if (sttIsListening) {
        // 모바일에서 조기 종료되는 경우 재시작해 연속 인식을 유지
        scheduleSttRestart();
      }
    };
