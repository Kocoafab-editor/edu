(function (global) {
  ...
  class SerialAdapter {
    ...
    async _ws_disconnect() {
      ...
      this.port = null;
      this._opened = false;
      this._status('연결 안됨', 'status-disconnected');
      return true;
    }
    ...
    _p5_ensure() {
      ...
      this.p5serial.on('close', () => { this._opened = false; this._status('연결 안됨', 'status-disconnected'); });
    ...
    async _p5_connect() {
      ...
      // 항상 requestPort를 호출해 사용자 선택을 강제
      if (typeof this.p5serial.requestPort === 'function') {
        await this.p5serial.requestPort();
      }
      // open 호출 (웹 시리얼 기반 p5.serialport는 requestPort 이후 open 필요)
